...
async def create_folder(
    session: SessionDep, folder_instance: schemas.FolderCreate = Body(...)
):
    db_folder = folder.create(session=session, folder=folder_instance)
...
async def add_candidate_to_folder(
    ...
    db_folder = folder.add_candidate_to_folder_by_id(
        session=session, folder_id=folder_id, candidate=candidate_instance
    )
...
async def get_folders(session: SessionDep):
    db_folders = folder.get_all(session=session)
